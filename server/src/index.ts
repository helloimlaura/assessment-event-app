 ...
 *
 *  The fallback is the Vite dev origin, not `port`: a registration link is a
 *  route only the SPA serves, so advertising the API port would encode a path
 *  Express has no handler for. Point `PUBLIC_BASE_URL` at a LAN address to scan
 *  from a real phone, or at the deployed origin in production. */
